...
})},__resize:function(){this.pageWidth=this.el.clientWidth;this.__pos(-this.page*this.pageWidth)},__start:function(f){this.el.style.webkitTransitionDuration="0";
this.isScrolling=false;this.__addExternalFunction(this.slideHandlers.onSlideStart);this.__initFlicking()
},__initFlicking:function(){this.startTime=new Date()},isFlicking:function(){this.endTime=new Date()-this.startTime;
if(this.endTime<200){return true}else{return false}},__move:function(f){if(f.isSwipe()&&!this.isScrolling){f.targetEvent.preventDefault();
this.__pos(-this.page*this.pageWidth+f.delta.x);this.__addExternalFunction(this.slideHandlers.onSlideMove)
}else{if(f.isScroll()){this.isScrolling=true}}},__end:function(f){this.flickState=this.isFlicking();if(this.__isNextSwipe(f)){this.__next()
}else{if(this.__isPrevSwipe(f)){this.__prev()}else{this.__cancel()}}this.__addExternalFunction(this.slideHandlers.onSlideEnd)
},__pos:function(f){this.el.style.webkitTransform="translate3d("+f+"px,0,0)"},__isNextSwipe:function(f){if(f.isLeft()&&((this.el.clientWidth*-1*e>f.delta.x)||this.flickState)){return true
}else{return false}},__isPrevSwipe:function(f){if(f.isRight()&&((this.el.clientWidth*e<f.delta.x)||this.flickState)){return true
}else{return false}},__next:function(){var g=this.dataSource.getNextData();if(g.type==="invalid"){this.__cancel()
}else{this.el.style.webkitTransitionDuration="500ms";this.__plusPageOffset();this.__pos(-this.page*this.pageWidth);
var f=this.__getNextOffsetOfMovingPanel();this.__movePanel(this.page+1,f);this.__setDataItem(f,g.data);
this.__addExternalFunction(this.slideHandlers.onSlideNext)}},__prev:function(){var g=this.dataSource.getPrevData();
if(g.type==="invalid"){this.__cancel()}else{this.el.style.webkitTransitionDuration="500ms";this.__minusPageOffset();
this.__pos(-this.page*this.pageWidth);var f=this.__getPrevOffsetOfMovingPanel();this.__movePanel(this.page-1,f);
this.__setDataItem(f,g.data);this.__addExternalFunction(this.slideHandlers.onSlidePrev)}},__setDataItem:function(f,g){this.panels[f].innerHTML=g
},__getNextOffsetOfMovingPanel:function(){var f=this.panels.length,g=this.offset+parseInt(f/2);if(g>=f){g=g-f
}return g},__getPrevOffsetOfMovingPanel:function(){var f=this.panels.length,g=this.offset-parseInt(f/2);
...
},__cancel:function(){this.el.style.webkitTransitionDuration="500ms";this.__pos(-this.page*this.pageWidth);
this.__addExternalFunction(this.slideHandlers.onSlideCancel)},__addExternalFunction:function(f){if(f){f()
}}};function c(i,g,h){var j={onSlideStart:null,onSlideMove:null,onSlideEnd:null,onSlidePrev:null,onSlideNext:null,onSlideCancel:null};
...
},onSlideNext:function(k){j.onSlideNext=k},onSlideCancel:function(k){j.onSlideCancel=k},nextSlide:function(){f.__next()
},prevSlide:function(){f.__prev()}}}b.slideListener=c})(window.slide={});(function(a){var b=function(f,d,e){this.init(f,d,e)
};b.prototype={init:function(f,d,e){this.dataHandlers=f;this.isIterating=d||false;this.loadIndex=e||0;
this.dataList=[]},addData:function(d){this.dataList.push(d)},addDataList:function(d){this.dataList=this.dataList.concat(d)
},setDataTotalLength:function(d){this.totalLen=d},getDataTotalLength:function(){return this.totalLen||this.getDataListLength()
},getDataListLength:function(){return this.dataList.length},getDataByIndex:function(d){if(this.isIterating){d=this.__iterationIndexing(d)
}return this.__checkData(d)},__checkData:function(d){var e={};if(d===0){e.type="start";e.data=this.dataList[d]
}else{if(d===this.getDataTotalLength()-1){e.type="end";e.data=this.dataList[d]}else{if(d===-1){e.type="startPrev";
e.data=""}else{if(d===this.getDataTotalLength()){e.type="endNext";e.data=""}else{if(d>0&&d<this.getDataTotalLength()-1){e.type="valid";
e.data=this.dataList[d]}else{e.type="invalid";e.data=""}}}}}return e},__iterationIndexing:function(e){var d=this.getDataTotalLength();
if(e<0){do{e=d+e}while(e<0)}else{if(e>=d){do{e=e-d}while(e>=d)}}return e},getPrevData:function(){var d=this.getDataByIndex(this.getPrevIndex());
