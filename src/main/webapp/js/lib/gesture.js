...
this.startPos=a(j,i);this.delta=a(0,0);this.targetEvent=null;this.term=0;this.startTime=new Date()}g.prototype={};
g.prototype.isSwipe=function(){return this.type==="swipe"};g.prototype.isScroll=function(){return this.type==="scroll"
...
};g.prototype.isFlick=function(){return this.term<200?true:false};g.prototype.getTerm=function(){return this.term
};g.prototype.identifyWithCurrentPos=function(i,l){var j=a(i,l);var k=this.delta=j.minus(this.startPos);
if(this.type==="unindentified"){if(Math.abs(k.x)>c&&Math.abs(k.x)>=Math.abs(k.y)){this.type="swipe";if(k.x<0){this.direction="left"
}else{this.direction="right"}}else{if(Math.abs(k.y)>c&&Math.abs(k.y)>Math.abs(k.x)){this.type="scroll";
if(k.y<0){this.direction="up"}else{this.direction="down"}}}}};g.prototype.checkFlick=function(){this.term=new Date-this.startTime
};function a(i,j){return{x:i,y:j,minus:function(k){return a(this.x-k.x,this.y-k.y)}}}function e(i){return(document.addEventListener)?i:"on"+i
}var h={START:function(){return("ontouchstart" in window)?"touchstart":e("mousedown")}(),MOVE:function(){return("ontouchstart" in window)?"touchmove":e("mousemove")
}(),END:function(){return("ontouchstart" in window)?"touchend":e("mouseup")}(),listen:function(){if(document.addEventListener){return function(k,j,i){k.addEventListener(j,function(l){i.call(k,l)
